...
function tryGitCommit(appPath) {
  try {
    ...
    execSync('git commit -m "build(init): 使用@wanglihua/create-ezreal-app初始化项目"', {
      stdio: 'ignore',
    })
    return true
  } catch (e) {
...
module.exports = function (appPath, appName, verbose, originalDirectory, templateName) {
  ...
  const devDependenciesToInstall = Object.entries(templatePackage.devDependencies || {})
  ...
  if (templateName && devDepArgs.length > 1) {
    ...
    const proc = spawn.sync(command, [...devDepArgs, useYarn ? '--dev' : '--save-dev'], {
      stdio: 'inherit',
    })
    if (proc.status !== 0) {
